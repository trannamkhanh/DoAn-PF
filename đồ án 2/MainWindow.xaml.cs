...
namespace WpfApp3
{
    public partial class MainWindow : Window
    {
        ...
        {
            NavigateToTab(0);
            UpdateSidebarStyles(sender as Button);
        }
        ...
        private void UpdateSidebarStyles(Button clickedButton)
        {
            if (clickedButton == null) return;

            // Reset all sidebar buttons to default style
            BtnSidebarHome.Style = (Style)FindResource("SideMenuButtonStyle");
            BtnSidebarBooks.Style = (Style)FindResource("SideMenuButtonStyle");
            BtnSidebarMembers.Style = (Style)FindResource("SideMenuButtonStyle");
            BtnSidebarLoans.Style = (Style)FindResource("SideMenuButtonStyle");
            BtnSidebarReports.Style = (Style)FindResource("SideMenuButtonStyle");

            // Set clicked button to active style
            clickedButton.Style = (Style)FindResource("SideMenuButtonActiveStyle");
        }
        ...
        {
            // Hiển thị tổng số "Đầu sách" (số lượng tựa sách) thay vì tổng số cuốn (Quantity)
            TxtHomeTotalBooks.Text = _db.Books.Count().ToString();
